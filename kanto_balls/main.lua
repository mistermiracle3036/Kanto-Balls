...
return function(mod)
  ...
  if GEN2 then
    ...
    ----------------------------------------------------------------------
    -- THE RESCUE FLAG -- READ BY NAME, not derived.
    --
    -- EVENT_CLEARED_SLOWPOKE_WELL is what gates Kurt's gift, and the
    -- engine already ships its number:
    --   engine/src/core/gen2/FlagNames.lua:561  -> 43
    -- generated by tools/goldwalk/gen_flags.lua from pret/pokegold's own
    -- constants/event_flags.asm and checked against the ROM.
    --
    -- 0.8.4 through 0.8.9 DERIVED this number instead, by walking Kurt's
    -- decompiled script for a `checkevent` whose branch reached a give.
    -- Sixty lines of heuristic to rediscover a constant the engine names
    -- three directories away, and it got the answer WRONG twice on the
    -- way -- first event 53 (the gift already handed over), then only
    -- landing on 43 after a tie-break on branch depth. This repo's own
    -- rule covers it: never guess a ROM constant, READ it. That rule
    -- names rom_manifest.json for TEXT_ ids; FlagNames.lua is the same
    -- thing for event flags and I did not look for it.
    --
    -- Confirmed independently: Crystal's own extracted script has
    -- 63:6178 `checkevent 43` -> 63:61bf, the branch that hands over the
    -- LURE BALL. So 43 is right on both lineages, from two sources.
    --
    -- Resolved ONCE at load, not per game.ready: it is a ROM constant,
    -- not save state. pcall'd because a future engine could move the
    -- module, and a nil flag simply falls through to the bag diff below
    -- rather than breaking the handover.
    local KURT_FLAG = nil
    do
      local ok, FlagNames = pcall(require, "src.core.gen2.FlagNames")
      local id = ok and type(FlagNames) == "table" and FlagNames.events
        and FlagNames.events.EVENT_CLEARED_SLOWPOKE_WELL
      if type(id) == "number" then
        KURT_FLAG = id
      else
        Runtime.reportError("kanto_balls", "KURT flag name unresolved")
      end
    end
